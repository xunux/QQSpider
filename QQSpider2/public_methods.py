    ...
    while failure < 2:
        try:
            ...
            while '验证码' in browser.page_source:
                try:
                    ...
                    try:
                        ...
                        log.error('验证码识别结果: %s', verification_code)
            ...
            if browser.title == 'QQ空间':
                ...
                log.error('Get the cookie of QQ:%s successfully!(共%d个键值对)', account, len(cookie))
                browser.quit()
                return json.dumps(cookie)  # 将字典转成字符串
            else:
                log.error('Get the cookie of QQ:%s failed!', account)
                return ''
        except Exception, e:
            failure = failure + 1
            if 'browser' in dir():
